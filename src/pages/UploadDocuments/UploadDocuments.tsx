...
export default function UploadDocuments() {
  ...
  const handleCreateRecord = async () => {
    try {
      ...
      if (!deviceId) {
        setMessage("Device not found. Please login again.");
        return;
      }
      ...
      if (mode === "file") {
        ...
        recordData = {
          title: "Smart Corpus Management Report",
          description:
          ...
          file_name: file.name,
          file_size: Math.max(file.size, 1024),
          status: "pending",
          reviewed: false,
          source_label: "Frontend Upload",
        ...
        recordData = {
          ...
          media_type: "text",
          file_url: "https://example.com/text-document.txt",
          file_name: `${title}.txt`,
          file_size: Math.max(content.length, 1024),
          status: "pending",
          reviewed: false,
          source_label: "Text Upload",
          source_url: "https://example.com",
          user_id: userId,
          category_ids: [],
          tagged_usernames: [],
      ...
      await api.post("/api/v1/records", recordData);
